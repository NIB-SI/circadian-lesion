...
Mat findLesions2(Mat contrast, Mat input) //day images
{
	...
	Mat dil;
	dilate(hue_range, dil, Mat(), cv::Point(-1,-1), 11);
	erode(dil, dil, Mat(), cv::Point(-1,-1), 5);
	...
	vector<vector<int>> count;
		...
		if(total_brightness[i][0]<=75)
		{
			...
			for ( size_t j = 0; j < contours1.size(); j++)
			{
				...
				if( contourArea(contours1[j]) < 175 || (mc.x-10)<0 || (mc.y-10)<0 || (mc.x+10)>input.cols || (mc.y+10)>input.rows)
				{
