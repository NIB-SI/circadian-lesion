...
Mat findLesions1(Mat contrast, Mat input) //night images
{
	...
	cv::inRange(hsv, cv::Scalar(0, 0, 0), cv::Scalar(80, 255, 185), hue_range);
...
Mat findLesions2(Mat contrast, Mat input) //day images
{
	...
	cv::inRange(hsv, cv::Scalar(0, 20, 40), cv::Scalar(30, 255, 230), hue_range1);
...
int main( int argc, char** argv )
{
	...
	vector<vector<int>> count;
		...
		if(i==0)
		{
			...
			lesion.insert(lesion.end(), {2445,2083,3271,4914,5920,4966,4992,514,697,346,301,323,647,387});
			mp.insert(mp.end(), {Point(1063, 720),Point(1199, 636),Point(788, 524),Point(165, 485),Point(216, 383),Point(1171, 303),Point(1108, 156),Point(370, 492),Point(265, 513),Point(265, 513),Point(562, 683),Point(715, 548),Point(694, 375),Point(981, 220)});
			allRadii.insert(allRadii.end(), {36,35,40,47,48,54,48,19,27,27,12,15,18,15});
			allCenters.insert(allCenters.end(), {Point2f(1020.5, 730),Point2f(1163, 648.5),Point2f(747.085, 536.305),Point2f(124.699, 497.42),Point2f(173.946, 395.71),Point2f(1130.5, 314.5),Point2f(1071, 166.5),Point2f(330.5, 507.5),Point2f(224.395, 523.29),Point2f(224.395, 523.29),Point2f(522.5, 694.5),Point2f(677, 558.5),Point2f(654, 383.5),Point2f(943.5, 232.5)});
		}		
		...
		{		
			if(total_brightness[i][0]<=75)
			{
				...
				for ( size_t j = 0; j < contours1.size(); j++)
				{
					...
					if( contourArea(contours1[j]) < 100 || (mc.x-50)<0 || (mc.y-50)<0 || (mc.x+50)>input.cols || (mc.y+50)>input.rows)
					{
			...
			else
			{
				...
				for ( size_t j = 0; j < contours.size(); j++)
				{
					...
					//for contours with too small area
					if( contourArea(contours[j]) < 50 || (mc.x-50)<0 || (mc.y-50)<0 || (mc.x+50)>input.cols || (mc.y+50)>input.rows)
					{
