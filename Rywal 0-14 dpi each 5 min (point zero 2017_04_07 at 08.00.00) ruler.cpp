...
Mat findLesions1(Mat contrast, Mat input)
{
	Mat hsv;
...
Mat findLesions2(Mat contrast, Mat input)
{
	Mat hsv;
	cvtColor(contrast,hsv,COLOR_BGR2HSV);
	...
	cv::inRange(hsv, cv::Scalar(170, 20, 40), cv::Scalar(180, 255, 235), hue_range2);
	...
	vector<vector<int>> count;
		...
		if(total_brightness[i][0]<=125)
		{
			...
			for ( size_t j = 0; j < contours1.size(); j++)
			{
				...
				if( contourArea(contours1[j]) < 200 || (mc.x-50)<0 || (mc.y-50)<0 || (mc.x+50)>input.cols || (mc.y+50)>input.rows)
				{
