...
Mat findLesions2(Mat contrast, Mat input) //day images
{
	...
	cv::inRange(hsv, cv::Scalar(0, 20, 40), cv::Scalar(25, 255, 232), hue_range1);
	cv::inRange(hsv, cv::Scalar(170, 20, 40), cv::Scalar(180, 255, 230), hue_range2);
...
int main( int argc, char** argv )
{
	...
	vector<vector<int>> count;
		...
		if(i==0)
		{
			...
			lesion.insert(lesion.end(), {1193,3976,493,4470,501,3670,1258,5300,1300,1105,2718,1701,5391,760});
			mp.insert(mp.end(), {Point(242, 783),Point(180, 738),Point(61, 533),Point(491, 519),Point(395, 410),Point(280, 412),Point(421, 362),Point(178, 377),Point(178, 377),Point(541, 292),Point(905, 302),Point(894, 210),Point(960, 160),Point(203, 32)});
			allRadii.insert(allRadii.end(), {28,52,15,52,14,40,26,74,74,23,36,30,57,20});
			allCenters.insert(allCenters.end(), {Point2f(245.682, 779.408),Point2f(177.5, 738.5),Point2f(59.8708, 533.646),Point2f(487.5, 516.5),Point2f(395.743, 409.919),Point2f(284.894, 414.905),Point2f(418.214, 363.996),Point2f(182.5, 371.5),Point2f(182.5, 371.5),Point2f(540.924, 293.561),Point2f(906.5, 303),Point2f(889.312, 206.327),Point2f(961.5, 161.5),Point2f(203.072, 29.6824)});
		}		
		...
		if(i!=0)
		{
			if(total_brightness[i][0]<=100)
			{
				...
				for ( size_t j = 0; j < contours1.size(); j++)
				{
					...
					if( contourArea(contours1[j]) < 175 || (mc.x-25)<0 || (mc.y-25)<0 || (mc.x+25)>input.cols || (mc.y+25)>input.rows)
					{
					...
					{
						drawContours(input, contours1, j, cv::Scalar(0, 0, 505),1, 8);
						boundRect = boundingRect( Mat(contours1[j]) );
						//rectangle(input, boundRect.tl(), boundRect.br(), Scalar(0, 0, 505),2,8,0); 				

						minEnclosingCircle( contours1[j], center, radius );					
						circle( input, center, radius, Scalar(0, 0, 505),2, 8, 0 );
			...
			else
			{
				...
				for ( size_t j = 0; j < contours.size(); j++)
				{
					...
					//for contours with too small area
					if( contourArea(contours[j]) < 150|| (mc.x-25)<0 || (mc.y-25)<0 || (mc.x+25)>input.cols || (mc.y+25)>input.rows)
					{
	...
	for (int i = 0; i < lesion.size(); i++)
	{
		...
		else
		{
			...
			{	
				...
				if(smallest_dist<75) //ignore too far lesions (forget them)
				{
	...
	for ( size_t m = 0; m < lesion.size(); m++)
	{
		...
		imwrite("/home/jpfeil/Leaves/results/RywalNahG 4-10 dpi each 20 min (point zero 2016_11_04 at 06.00.00)/"+filename, input);
