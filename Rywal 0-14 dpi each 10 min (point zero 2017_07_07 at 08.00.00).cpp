...
Mat findLesions1(Mat contrast, Mat input) //night images
{
	...
	cv::inRange(hsv, cv::Scalar(0, 0, 0), cv::Scalar(80, 255, 150), hue_range);
...
Mat findLesions2(Mat contrast, Mat input) //day images
{
	...
	cv::inRange(hsv, cv::Scalar(0, 20, 40), cv::Scalar(20, 255, 220), hue_range1);
	...
	vector<vector<int>> count;
		...
		else
		{
			...
			for ( size_t j = 0; j < contours.size(); j++)
			{
				...
				//for contours with too small area
				if( contourArea(contours[j]) < 100 || (mc.x-50)<0 || (mc.y-50)<0 || (mc.x+50)>input.cols || (mc.y+50)>input.rows)
				{
