...
Mat findLesions1(Mat contrast, Mat input) //night images
{
	...
	cv::inRange(hsv, cv::Scalar(0, 0, 0), cv::Scalar(80, 255, 190), hue_range);
	...
	vector<vector<int>> count;
		...
		if(total_brightness[i][0]<=75)
		{
			...
			for ( size_t j = 0; j < contours1.size(); j++)
			{
				...
				if( contourArea(contours1[j]) < 50|| (mc.x-15)<0 || (mc.y-15)<0 || (mc.x+15)>input.cols || (mc.y+15)>input.rows)
				{
		...
		else
		{
			...
			for ( size_t j = 0; j < contours.size(); j++)
			{
				...
				//for contours with too small area
				if( contourArea(contours[j]) < 50|| (mc.x-25)<0 || (mc.y-25)<0 || (mc.x+25)>input.cols || (mc.y+25)>input.rows)
				{
