...
Mat findLesions1(Mat contrast, Mat input) //night images
{
	...
	cv::inRange(hsv, cv::Scalar(0, 0, 0), cv::Scalar(80, 255, 140), hue_range);
...
Mat findLesions2(Mat contrast, Mat input) //day images
{
	...
	cv::inRange(hsv, cv::Scalar(0, 20, 40), cv::Scalar(25, 255, 200), hue_range1);
	cv::inRange(hsv, cv::Scalar(170, 20, 40), cv::Scalar(180, 255, 200), hue_range2);
...
int main( int argc, char** argv )
{
	...
	vector<vector<int>> count;
		...
		//cout<<total_brightness[i][0]<<endl;

		if(total_brightness[i][0]<=145)
		{
	...
	for (int i = 0; i < lesion.size(); i++)
	{
		...
		else
		{
			...
			if(total_brightness[i][0]>=145) //daylight images
			{ 
			...
			{	
				...
				{				
						...
						if(total_brightness[i-1][0]>=145) //precursor image is bright
	...
	for (int j = 0; j < all_growth.size(); j++) 
	{
		...
		{	
			if(total_brightness[j][0]<=145){
			if(total_brightness[j-1][0]>=145){lesion[j][k]=all_growth[j][k];} 
	...
	for (int i = 0; i < lesion.size(); i++)
	{	
		if(total_brightness[i][0]<=145){cout<<"\033[1;31m"<<timepoints[i]<<"\t"<<taf[i]<<"\033[0m"<<"\t";}
