	...
	vector<vector<int>> count;
		...
		if(total_brightness[i][0]<=125)
		{
			...
			for ( size_t j = 0; j < contours1.size(); j++)
			{
				...
				if( contourArea(contours1[j]) < 350 || (mc.x-50)<0 || (mc.y-50)<0 || (mc.x+50)>input.cols || (mc.y+50)>input.rows)
				{
		...
		else
		{
			...
			for ( size_t j = 0; j < contours.size(); j++)
			{
				...
				//for contours with too small area
				if( contourArea(contours[j]) < 350 || (mc.x-75)<0 || (mc.y-75)<0 || (mc.x+75)>input.cols || (mc.y+75)>input.rows)
				{
