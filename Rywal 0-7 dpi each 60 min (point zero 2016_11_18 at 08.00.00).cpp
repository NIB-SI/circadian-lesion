...
Mat findLesions1(Mat contrast, Mat input) //night images
{
	...
	cv::inRange(hsv, cv::Scalar(0, 0, 0), cv::Scalar(80, 255, 200), hue_range);
...
Mat findLesions2(Mat contrast, Mat input) //day images
{
	...
	cv::inRange(hsv, cv::Scalar(0, 20, 40), cv::Scalar(25, 255, 225), hue_range1);
...
int main( int argc, char** argv )
{
	...
	for (int i = 0; i < lesion.size(); i++)
	{
		...
		else
		{
			...
			{	
				...
				if(smallest_dist<175) //ignore too far lesions (forget them)
				{
			...
			if(total_brightness[i][0]>=115) //daylight images
			{ 
			...
			{	
				...
				{				
						...
						if(total_brightness[i-1][0]>=115) //precursor image is bright
	...
	for (int j = 0; j < all_growth.size(); j++) 
	{
		...
		{	
			if(total_brightness[j][0]<=115){
			if(total_brightness[j-1][0]>=115){lesion[j][k]=all_growth[j][k];} 
	...
	for (int i = 0; i < lesion.size(); i++)
	{	
		if(total_brightness[i][0]<=115){cout<<"\033[1;31m"<<timepoints[i]<<"\t"<<taf[i]<<"\033[0m"<<"\t";}
