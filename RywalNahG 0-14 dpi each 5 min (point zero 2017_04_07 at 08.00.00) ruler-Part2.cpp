...
Mat findLesions1(Mat contrast, Mat input) //night images
{
	...
	cv::inRange(hsv, cv::Scalar(0, 0, 0), cv::Scalar(80, 255, 175), hue_range);
...
int main( int argc, char** argv )
{
	...
	vector<vector<int>> count;
		...
		if(i==0)
		{
			count.insert(count.end(), {0,1,2,3,4});												
			lesion.insert(lesion.end(), {6560,4422,3445,4178,250});
			mp.insert(mp.end(), {Point(256, 473),Point(1045, 811),Point(668, 684),Point(642, 179),Point(700,170)});
			allRadii.insert(allRadii.end(), {55,60,45,62,30});
			allCenters.insert(allCenters.end(), {Point2f(254.74, 470.22),Point2f(1049.5, 818.5),Point2f(669.664, 681.44),Point2f(650.305, 174.5),Point2f(700,170)});
		}
		...
		if(i!=0)
		{
			if(total_brightness[i][0]<=125)
			{
				...
				for ( size_t j = 0; j < contours1.size(); j++)
				{
					...
					if( contourArea(contours1[j]) < 250 || (mc.x-50)<0 || (mc.y-50)<0 || (mc.x+50)>input.cols || (mc.y+50)>input.rows)
					{
	...
	for ( size_t m = 0; m < lesion.size(); m++)
	{
		...
		imwrite("/home/jpfeil/Leaves/results/RywalNahG 0-14 dpi each 5 min (point zero 2017_04_07 at 08.00.00) ruler/"+filename, input);
